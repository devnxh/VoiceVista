...
def process_short_video(video_path, target_language):
    ...
    try:
        ...
        # Split the translated text into sentences for better TTS results
        sentences = []
        for end_char in ['. ', '! ', '? ', '.\n', '!\n', '?\n']:
            translated_text = translated_text.replace(end_char, end_char + '<SPLIT>')
        
        raw_sentences = translated_text.split('<SPLIT>')
        sentences = [s.strip() for s in raw_sentences if s.strip()]
        logger.info(f"Split translation into {len(sentences)} sentences for TTS")
        ...
        for i, sentence in enumerate(sentences):
            ...
            sentence_filename = f"sentence_{i}_{output_id}.mp3"
            sentence_path = os.path.join(app.config['OUTPUT_FOLDER'], sentence_filename)
            
            try:
                logger.info(f"Generating TTS for sentence {i+1}/{len(sentences)}")
                tts = gTTS(text=sentence, lang=target_language, slow=False)
                tts.save(sentence_path)
                sentence_audio_files.append(sentence_path)
            except Exception as e:
                logger.error(f"Error generating TTS for sentence {i+1}: {e}")
        
        # Combine all sentence audio files sequentially
        combined_audio_filename = f"translated_audio_{output_id}.mp3"
        combined_audio_path = os.path.join(app.config['OUTPUT_FOLDER'], combined_audio_filename)
        ...
        # Clean up individual sentence audio files
        for audio_file in sentence_audio_files:
        ...
        audio_to_use = combined_audio_path
        if abs(audio_duration - video_duration) > 1.0:  # Only adjust if difference > 1 second
            ...
            if adjust_audio_speed(combined_audio_path, video_duration, speed_adjusted_audio):
        ...
        if audio_to_use != combined_audio_path:
...
def process_long_video(video_path, target_language, duration):
    ...
    try:
        ...
        for i in range(chunk_count):
            ...
            if whisper_model:
                result = whisper_model.transcribe(chunk_audio_path)
                transcription = result["text"]
            ...
            success = synthesize_speech_safely(translated_text, chunk_translated_path, target_language)
            if not success:
            ...
            # Add to the list of audio segments
            translated_audio_segments.append(chunk_translated_path)
            
            # Delete the temporary WAV file to save space
            os.remove(chunk_audio_path)
        
        # Combine all audio segments sequentially
        logger.info("Combining audio segments sequentially...")
        ...
        # Use ffmpeg concat demuxer to join files in sequence
        concat_cmd = [
            "ffmpeg",
            "-f", "concat",
            "-safe", "0",
        ...
        subprocess.run(concat_cmd, check=True)
        logger.info(f"Sequential audio created at {combined_audio}")
        ...
        # Merge final audio with the original video
        logger.info("Merging final audio with original video")
        subprocess.run([
            "ffmpeg",
            "-i", video_path,
            "-i", audio_to_use,
